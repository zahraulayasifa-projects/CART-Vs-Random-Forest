...
library(rpart) 
model.pohon <- rpart(as.factor(default) ~ age + ed + employ + address + income + debtinc + creddebt + othdebt, data=bankloan.training) 
...
set.seed(100) 
model.forest <- randomForest(as.factor(default) ~ age + ed + employ + address + income + debtinc + creddebt + othdebt,  data=bankloan.training, importance=TRUE, ntree=2000, mtry=3) 
...
mean_employ <- mean(bankloan$employ[bankloan$employ != 0], na.rm = TRUE) 
mean_address <- mean(bankloan$address[bankloan$address != 0], na.rm = TRUE) 
